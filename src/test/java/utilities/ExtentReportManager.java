...
public class ExtentReportManager implements ITestListener{ //ITestListener is a testng method used in post action after tests are executed
	...
	public void onStart(ITestContext testContext) {
		...
		sparkReporter= new ExtentSparkReporter("target/cucumber-reports/"+repName); //specify location of the report
		
		sparkReporter.config().setDocumentTitle("LumaWebsiteAutomationProject"); //Title of Report
		sparkReporter.config().setReportName("WEB AUTOMATION REPORT"); //name of the report
		sparkReporter.config().setTheme(Theme.DARK);
		
		extent= new ExtentReports();
		extent.attachReporter(sparkReporter);
		extent.setSystemInfo("Application", "Luma Website Page");
